...
impl Interpreter{
    fn preprocess(&self, input: String, preprocessed_str: &mut Vec<String>, is_func: bool){
    ...
    fn tokenize(&self, preprocessed_str: Vec<String>, token_stream: &mut Vec<Token>, is_func: bool) -> Option<&'static str>{
        // Some predefined funtions
        ...
        for (i, item) in preprocessed_str.iter().enumerate(){
            ...
            else if self.vars.contains_key(&item.as_str()){
                token_stream.push(Token{
                    id: TokenId::Num,
                    value: self.vars[&item.as_str()].to_string(),
                    num: self.vars[&item.as_str()],
                });
            }
                        ...
                        match token_stream.last(){
                            ...
                            Some(n) => {
                                match n.id{
                                    ...
                                    TokenId::Num | TokenId::ClosedParanthese | TokenId::XVar=> {
                                        token_stream.push(Token{
                                            id: TokenId::Operator,
                                            value: String::from("-"),
                                            num: 1.0,
    ...
    fn infix_to_postfix(&self, token_stream: Vec<Token>, postfix_equation: &mut Vec<Token>) -> Option<&'static str>{
        let mut stack: Vec<Token> = Vec::new();
        for token in token_stream{
            match token.id{
                TokenId::Num | TokenId::XVar => postfix_equation.push(token),
    ...
    pub fn evaluate(&self, equation: Vec<Token>, result: &mut f64) -> Option<&'static str>{
                ...
                TokenId::Operator => {
                    ...
                    match token.value.as_str(){
                        ...
                        "/" => {
                            let res = val1/val2;
                            if res.is_infinite(){
                                return Some("Divide by zero error");
                            }
                            else{
                                stack.push(res);
                            }
                        ...
                        "root" => stack.push(val2.powf(val1.recip())),
    ...
    pub fn get_tokens(&self, mut input: String) -> Result<Vec<Token>, &'static str>{
        // remove spaces so that they don't cause pain
        input.retain(|c| c != ' ');
        let mut preprocessed_str: Vec<String> = Vec::new();
        self.preprocess(input, &mut preprocessed_str, true);

        //make a token stream for easier proccessing
        let mut token_stream: Vec<Token> = Vec::new();
        match self.tokenize(preprocessed_str, &mut token_stream, true){
            None => (),
            Some(n) => return Err(n),
        }
        ...
        match self.infix_to_postfix(token_stream, &mut postfix_equation){
            None => (),
            Some(n) => return Err(n),
        }
        return Ok(postfix_equation);
    }

    pub fn solve(&self, mut input: String) -> Result<f64, &'static str>{
        // remove spaces so that they don't cause pain
        input.retain(|c| c != ' ');
        let mut preprocessed_str: Vec<String> = Vec::new();
        self.preprocess(input, &mut preprocessed_str, false);

        //make a token stream for easier proccessing
        let mut token_stream: Vec<Token> = Vec::new();
        match self.tokenize(preprocessed_str, &mut token_stream, false){
            None => (),
            Some(n) => return Err(n),
        }
        ...
        let mut res: f64 = 0.0;
        match self.evaluate(postfix_equation, &mut res){
            None => (),
            Some(n) => return Err(n),
        }
        if res.is_infinite(){
